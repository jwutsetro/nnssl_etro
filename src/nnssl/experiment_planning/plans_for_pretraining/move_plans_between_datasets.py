...
def move_plans_between_datasets(
    ...
    source_folder = join(nnssl_preprocessed, source_dataset_name)
    ...
    rw = determine_reader_writer_from_dataset_json(
        target_dataset_json, example_image, allow_nonmatching_filename=True, verbose=False
    )
    ...
    save_json(
        source_plans,
        join(nnssl_preprocessed, target_dataset_name, target_plans_identifier + ".json"),
        sort_keys=False,
    )
...
def entry_point_move_plans_between_datasets():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Source plans identifier. If your plans are named "nnsslPlans.json" then the '
        "identifier would be nnsslPlans",
    )
    parser.add_argument(
        "-tp",
        type=str,
...
if __name__ == "__main__":
    move_plans_between_datasets(2, 4, "nnsslPlans", "nnsslPlansFrom2")
