...
class nnsslPredictor(object):
    ...
    def initialize_from_trained_model_folder(
        ...
        for i, f in enumerate(use_folds):
            ...
            checkpoint = torch.load(
                join(model_training_output_dir, f"fold_{f}", checkpoint_name), map_location=torch.device("cpu")
            )
            if i == 0:
        ...
        network = trainer_class.build_network_architecture(
            plans_manager, dataset_json, configuration_manager, num_input_channels, enable_deep_supervision=False
        )
    ...
    def get_data_iterator_from_raw_npy_data(
        ...
        list_of_images = (
            [image_or_list_of_images] if not isinstance(image_or_list_of_images, list) else image_or_list_of_images
        )
    ...
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        ...
        if len(self.configuration_manager.patch_size) < len(image_size):
            ...
            steps = compute_steps_for_sliding_window(
                image_size[1:], self.configuration_manager.patch_size, self.tile_step_size
            )
            if self.verbose:
                print(
                        ...
                        slicers.append(
                            tuple(
                                [
                                    ...
                                    *[
                                        slice(si, si + ti)
                                        for si, ti in zip((sx, sy), self.configuration_manager.patch_size)
                                    ],
                                ]
                            )
                        )
                        ...
                        slicers.append(
                            tuple(
                                [
                                    ...
                                    *[
                                        slice(si, si + ti)
                                        for si, ti in zip((sx, sy, sz), self.configuration_manager.patch_size)
                                    ],
                                ]
                            )
                        )
    ...
    def _internal_predict_sliding_window_return_logits(
        ...
        predicted_logits = torch.zeros(
            (self.label_manager.num_segmentation_heads, *data.shape[1:]), dtype=torch.half, device=results_device
        )
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument(
        ...
        required=False,
        default=(0, 1, 2, 3, 4),
        help="Specify the folds of the trained model that should be used for prediction. " "Default: (0, 1, 2, 3, 4)",
    )
    parser.add_argument(
        "-step_size",
        type=float,
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Set this flag to disable progress bar. Recommended for HPC environments (non interactive " "jobs)",
    )
